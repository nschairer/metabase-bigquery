...
(defmethod ->temporal-type :default
  ...
  (cond
    ...
    :else
    (let [hsql-form     (sql.qp/->honeysql :bigquery_alt x)
...
(defmethod unprepare/unprepare-value [:bigquery_alt LocalDate]
  [_ t]
  (format "date \"%s\"" (u.date/format-sql t)))

(defmethod unprepare/unprepare-value [:bigquery_alt LocalDateTime]
  [_ t]
  (format "datetime \"%s\"" (u.date/format-sql t)))

(defmethod unprepare/unprepare-value [:bigquery_alt OffsetTime]
  [_ t]
  ;; convert to a LocalTime in UTC
  (let [local-time (t/local-time (t/with-offset-same-instant t (t/zone-offset 0)))]
    (format "time \"%s\"" (u.date/format-sql local-time))))

(defmethod unprepare/unprepare-value [:bigquery_alt OffsetDateTime]
  [_ t]
  (format "timestamp \"%s\"" (u.date/format-sql t)))

(defmethod unprepare/unprepare-value [:bigquery_alt ZonedDateTime]
  [_ t]
...
(defmethod sql.qp/->honeysql [:bigquery_alt :asc]  [driver clause] (alias-order-by-field driver clause))
(defmethod sql.qp/->honeysql [:bigquery_alt :desc] [driver clause] (alias-order-by-field driver clause))
...
(doseq [filter-type [:between := :!= :> :>= :< :<=]]
  (defmethod sql.qp/->honeysql [:bigquery_alt filter-type]
    [driver clause]
...
(defmethod driver/mbql->native :bigquery_alt
  [driver
...
(defmethod sql.qp/current-datetime-honeysql-form :bigquery_alt
  [_]
  (CurrentMomentForm. nil))

(defmethod sql.qp/quote-style :bigquery_alt
  [_]
  :mysql)

;; convert LocalDate to an OffsetDateTime in UTC since BigQuery doesn't handle LocalDates as we'd like
(defmethod sql/->prepared-substitution [:bigquery_alt LocalDate]
  [driver t]
